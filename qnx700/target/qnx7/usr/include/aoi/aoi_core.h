...
enum AOConfigOptions {
	AO_CONFIG_ALL_DEFAULTS = 0,
	AO_CONFIG_CACHE_ALWAYS = 1,
};
...
int32_t AoSaveConfig(const char *config_file, uint32_t options);
int32_t AoLoadConfig(const char *config_file, AOLoadedConfig_t **cfg);
...
const char *AoFindString(const char * const *strings, const char *name);
const AOICtrl_t *AoAddDll(const char *dll, const char * const configs[], AOError_t *err);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/aoi/public/aoi/aoi_core.h $ $REV$")
#endif
