 ...
 *
 * This function clears an armed timer (a timer previously set by one of the
 * @c gesture_timer_set_*() functions). If the timer is unarmed, this function does nothing and returns 0.
 *
 * @param gesture   A pointer to the gesture.
 ...
 *
 * This function destroys the specified timer by resetting the timer data structure. It doesn't free
 * any memory associated with the timer.
 *
 * @param gesture   A pointer to the gesture.
 * @param timer_id  The timer to clear.
 *
 ...
 * @brief Query a timer
 *
 * This function returns information about the timer. If the timer is valid, the return value is set to 0,
 * the parameter @c pending is set to the timer's current pending state (destroyed timer will result
 * in a -1 return value), and @c expiry is set to the timer's expiry time.
 *
 * @param gesture        A pointer to the gesture.
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/gestures/public/gestures/timer.h $ $Rev: 812267 $")
#endif
