 ...
 */
 ...
 transition, for example:
   ...
   - Timer handling for gestures needing time events (as opposed to touch
       events)
...
/**
 *  @brief Detection of touch coordinates in the up direction.
 */
#define GESTURE_DIRECTION_UP (1 << 0)

/**
 *  @brief Detection of touch coordinates in the down direction.
 */
#define GESTURE_DIRECTION_DOWN (1 << 1)

/**
 *  @brief Detection of touch coordinates in the left direction.
 */
#define GESTURE_DIRECTION_LEFT (1 << 2)

/**
 *  @brief Detection of touch coordinates in the right direction.
 */
 ...
 *
 * This enumeration represents state of a gesture as it is
...
typedef enum {
	...
	/** The state of a gesture recognizer after it has been added to a
	*   gesture set; it is now ready to receive mtouch and timer events.
	*/
	GESTURE_STATE_UNRECOGNIZED,
	/** The state of a gesture recognizer after it has received one mtouch or
	*   timer event. This state is valid only for composite gestures.
	*/
	GESTURE_STATE_RECOGNIZED,
	/** The state of a gesture recognizer while it is receiving mtouch or
	*   timer events. This state is valid only for composite gestures.
	*/
	GESTURE_STATE_UPDATING,
	/** The state of a gesture recognizer when it has received all mtouch or
	*   timer events that fulfill the requirements of detecting its gesture.
	*/
	GESTURE_STATE_COMPLETE,
	/** The state of a gesture recognizer when requirements of detecting its
	*   gesture is not fulfilled.
	*/
 ...
 *
 * This data type consists of pointers to functions that handle mtouch events by
 * updating the gesture state, resetting the gesture data structures, or freeing
 * memory associated with the gesture. The following functions must be defined
 * for each of your custom gesture recognizer: 
 * - <tt>(*process_event)()</tt>
 * - <tt>(*reset)()</tt>
 * - <tt>(*free)()</tt>
 */
typedef struct {
	/**
	* @brief Free all the memory associated with the gesture that was
	*        allocated by the gesture's @c alloc() function. 
	*
	* @b Arguments
	* - @c gesture The gesture whose memory is to be freed.
	*
	*/
	void (*free)(struct gesture_base* gesture);

	/**
	* @brief Manage the state transitions and return the new, or unchanged,
	*        gesture state. 
	*
	* @b Arguments
	* - @c map Identifier used to identify the mtouch event.
	* - @c gesture Gesture recognizer whose state is being managed.
	* - @c event Mtouch event that has been received.
	* - @c consumed Indicator that gesture recognizer is finished recoginzing
	*               a gesture based on mtouch events received.
	*
	* @b Returns
	...
	*/
	...
	/**
	* @brief Reset the gesture-specific data structures to their initial state. 
	*
	* @b Arguments
	* - @c gesture Gesture recognizer whose specific data structures are to
	*              be returned to initial state.
	*
	*/
 ...
 *
 * This callback function is invoked every time a gesture
 ...
 * @param async   Indicator of which thread invoked this callback:
 *                 - @c async == 0: the thread that called @c gestures_set_process_event()
 *                 - @c async == 1: the timer thread
 *
 * @return Nothing.
 *
 */
 ...
 *
 * This structure represents information that is common to all
 ...
 *
 * This structure carries the x and y coordinates of a touch gesture,
 * as well as the time that it occured.
 *
 */
typedef struct {
	_Int32t x;             /**< The x coordinate of the touch. */
	_Int32t y;             /**< The y coordinate of the touch. */
	_Uint64t timestamp;    /**< The time when the touch occurred. */
} gesture_coords_t;

/**
 * @brief Initialize the gesture base data structure
 *
 * This function initializes the gesture base data structure, @c gesture_base_t.
 *
 ...
 *
 * This function adds a gesture to the 'must fail' list of another gesture.
 ...
 * 
 * This function saves the coordinates of a touch event in the 
 * specified @c gesture_coords_t data structure. You will likely need this
 * function only if you are defining your own gestures.
 *
 ...
 * 
 * This function returns the elapsed time between the two specified 
 * gesture events. You will likely need this function only if you are defining your
 * own gestures.
 *
 ...
 * 
 * This function returns the the maximum displacement, in pixels,
 ...
 * between the y coordinates, the former is returned by the function. You will likely need this
 * function only if you are defining your own gestures.
 *
 * @param coords1  A pointer to the first gesture event.
 * @param coords2  A pointer to the second gesture event.
 ...
 * 
 * This function remaps contact identifiers from touch events to 
 ...
 * sets in play, or if the user's finger is resting on the touch-sensitive bezel. You 
 * will likely need this function only if you are defining your own gestures.
 *
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/gestures/public/gestures/types.h $ $Rev: 812267 $")
#endif
