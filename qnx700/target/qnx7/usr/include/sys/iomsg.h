...
typedef union _io_link_extra {
	struct _msg_info32					info;		/* EXTRA_LINK (from client) */
...
typedef struct _io_mount_extra {
	...
	union {									/* if EXTRA_MOUNT_PATHNAME these set*/
		struct {							/* Sent from client to resmgr framework */
			struct _msg_info32	info;		/* special info on first mount, path info on remount */
		}					cl;
...
/*
 * Message of _IO_NOTIFY[64]
 */
struct _io_notify {
	/* NOTE: fields of _io_notify and _io_notify64 are intended to overlap.
	 * Don't modify one without modifying the other!
	 */
	_Uint16t					type;
	_Uint16t					combine_len;
	_Int32t						action;
...
struct _io_notify_reply {
	/* NOTE: fields of _io_notify_reply and _io_notify_reply64 are intended to overlap.
	 * Don't modify one without modifying the other!
	 */
	_Uint32t					zero;
...
/* for client:	If the flag is set, the client expects no change on offset
 *				as if  the server ignores the operation
 * for server:	To support the flag, server should not process lseek operation
 *				as if it ignores the operation
 *				For AIO support, non-seekable server should set errno = EOK in
 *				lseek handler
 */
#define _IO_LSEEK_IGNORE_NON_SEEKABLE	0x0001
...
/*
 * Message of _IO_UTIME[64]
 */
struct _io_utime {
	/* NOTE: fields of _io_utime and _io_utime64 are intended to overlap.
	 * Don't modify one without modifying the other!
	 */
	_Uint16t					type;
	_Uint16t					combine_len;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/c/public/sys/iomsg.h $ $Rev: 817442 $")
#endif
