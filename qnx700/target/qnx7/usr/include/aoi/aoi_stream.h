...
enum
{
  ...
  AO_STREAMER_DISCONTINUOUS = -6,   /* the read had a discontinuity in the data */
};
...
typedef struct StreamerStatistics_st {
  uint32_t    download_speed;// download speed in bits per seconds
  uint32_t    connect_time;  // connection time in microseconds
  uint32_t    time;          // time in microseconds of the last read
  uint32_t    nbytes;        // num bytes of the last read
...
#define AOSTREAMER_HTTP_AGENT_VERSION 101
/*
 * The AOStreamerHttpAgent interface is used to allow an alternate http streamer
 * to take over an input stream.
 * This interface is lookup by name in the http_streamer to optimize smooth streaming where the Microsoft SSPK needs to take over a http://... uri
 */
typedef struct AOStreamer AOStreamerHttpAgent;
...
typedef struct {
  ...
  AOIStream_t *(*reopen) ( const char* name, const char *mode, AOIStream_t *sobj, const AOParam_t *params_override, AOError_t *aoi_err);
...
#define AOSTREAMER_CONTROL_IO_VERSION   100

/*
 * This interface is provided by streamers that are capable of canceling blocking IO.
 * This is particularly useful for streamers that deal with a source/sink that can block
 * for a very long time or indefinitely.
 */
typedef enum
{
  AOSTREAMER_DISCARD_DATA = 0x01, /* Discard any data that maybe cached. */
} AOStreamerDisableFlags_t;

typedef struct
{
  /*  See flags above. */
  int32_t (*DisableIO)(void *ctx, AOStreamerDisableFlags_t type);
  /*  This function allows the re-enabling of IO. Enable can fail as disabling can require unrecoverable actions. */
  int32_t (*EnableIO)(void *ctx);
}  AOStreamerControlIO;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/aoi/public/aoi/aoi_stream.h $ $REV$")
#endif
