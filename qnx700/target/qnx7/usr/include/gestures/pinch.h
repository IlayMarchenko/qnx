 ...
 *
 * This structure is provided for consistency with other gesture
 * implementations. Although it carries no information, it cannot be empty because
 * common functions rely on its existence. 
 */
typedef struct {
	int mode;     /**< 0 = Default,  1 = Google Like */
} pinch_params_t;
...
typedef struct {
	...
	unsigned down_count;            /**< The the number of current touches. */
} gesture_pinch_t;

/**
 * @brief Allocate and initialize the pinch gesture structure
 *
 * This function allocates a new pinch gesture data structure,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/gestures/public/gestures/pinch.h $ $Rev: 816732 $")
#endif
