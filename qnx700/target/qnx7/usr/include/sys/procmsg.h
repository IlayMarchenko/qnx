...
enum {
	_PROC_SPAWN_START_REMOTE, /* Old _PROC_SPAWN_START, for backwards
								 compatibility */
	_PROC_SPAWN_FD,
	_PROC_SPAWN_ARGS,
	_PROC_SPAWN_DONE,
...
enum {
	_PROC_SANDBOX_CREATE,
	_PROC_SANDBOX_ATTACH,
	_PROC_SANDBOX_DETACH,
	_PROC_SANDBOX_DELETE
};
...
typedef union {
	struct _proc_wait				i;
	siginfo_t						o;
	struct _siginfo32				o32;
	struct _siginfo64				o64;
} proc_wait_t;
...
struct _proc_value {
	...
	_Uint64t						value;
	_Int32t							id;
	struct __sigevent32				event; /* Do NOT change this to a struct sigevent! */
};
...
struct _proc_sandbox_create_reply {
	_Uint32t	sbid;
};

typedef union {
	struct _proc_sandbox_create			i;
	struct _proc_sandbox_create_reply	o;
} proc_sandbox_create_t;

struct _proc_sandbox_attach {
	_Uint16t	type;
	_Uint16t	subtype;
	_Uint32t	sbid;
	_Uint32t	pid;
...
typedef union {
	struct _proc_sandbox_attach			i;
} proc_sandbox_attach_t;

struct _proc_sandbox_delete {
	_Uint16t	type;
	_Uint16t	subtype;
	_Uint32t	sbid;
};

struct _proc_sandbox_detach {
	_Uint16t	type;
	_Uint16t	subtype;
	_Uint32t	pid;
};
...
typedef union {
	struct _proc_sandbox_detach			i;
} proc_sandbox_detach_t;

typedef union {
	struct _proc_sandbox_delete			i;
} proc_sandbox_delete_t;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/services/system/public/sys/procmsg.h $ $Rev: 823588 $")
#endif
