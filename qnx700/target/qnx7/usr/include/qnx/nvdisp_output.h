...
#define NVDISP_MAX_OUTPUT_PER_HEAD  (2)
...
#define SCDC_I2C_DEV_NODE_LEN       (64)
...
/* This contain display output related configuration */
struct nvdisp_output {
    ...
    char scdc_i2c[SCDC_I2C_DEV_NODE_LEN];
    enum nvdisp_hotplug_level hotplug_level;
    unsigned int bpp;
    uint32_t lvds_lane_dc; //SOR_NV_PDISP_SOR_LANE_DRIVE_CURRENT0_0
    uint32_t lvds_lane4_dc; //SOR_NV_PDISP_SOR_LANE4_DRIVE_CURRENT0_0
    bool full_range; // Set to true for full color range
};

/* This contains display head related configuration */
struct nvdisp_head {
    int head_num;
...
/* This describe full display configuration i.e; Display head + Assosciated output(s) */
struct nvdisp_config {
    struct nvdisp_head head;
    struct nvdisp_output output[NVDISP_MAX_OUTPUT_PER_HEAD];
    int num_output;
};
