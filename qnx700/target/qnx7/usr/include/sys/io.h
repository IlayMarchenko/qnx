...
#define IO_FLUSH_NONBLOCK	0x1000	/* Flush should be non-blocking   */
...
#define IO_FLUSH_NOBLKNO	(-1ULL)	/* Device did not report bad blkno */
...
struct _mdl {
	paddr64_t	paddr;		/* Physical address */
...
struct _param {
	baddr_t	lba;			/* Logical block address                      */
...
struct _drvr_entry {
	...
	int			(*sync)(void *hdl, unsigned flag, baddr_t *bad);
};
...
#endif /* __NTO_IO_H_INCLUDED */

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/services/blk/io-blk/public/sys/io.h $ $Rev: 805416 $")
#endif
