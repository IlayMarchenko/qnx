...
/*
 * Message of _MEM_DEBUG_INFO[_64]
 */
struct _mem_debug_info {
	...
	_Uint32t						flags;
	dev_t							dev;
	_Uintptr32t						old_vaddr;
	__FLEXARY(char, path); /* char path[] */
};

struct _mem_debug_info64 {
	_Uint16t						type;
	_Uint16t						zero;
	_Uint32t						reserved;
	off64_t							offset;
...
typedef union {
	...
	struct _mem_swap_register		swap_register;
	struct _mem_swap_run			swap_run;
} mem_swap_t;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/services/system/public/sys/memmsg.h $ $Rev: 780356 $")
#endif
