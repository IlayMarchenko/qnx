...
typedef struct _iofunc_mount {
    ...
    /* Fields below here can only be considered initialized if (conf & IOFUNC_PC_EXT) */
    size_t                          size;       /* structure size */
    uint32_t                        ext_flags;  /* Extended Flags */
    uint32_t                        timeres;    /* timestamp resolution in ns */
    uint64_t                        reserved[4]; /* for future growth */
} iofunc_mount_t;
...
#define IOFUNC_PC_EXT               0x00000020  /* Structure is extended */
#define IOFUNC_PC_SYMLINK           0x00000040  /* Filesystem supports symlinks */

#define IOFUNC_PC_EXT_PERSISTTIME   0x00000001  /* Persisted time matches in-RAM time */


/***********************************************************************
 * File or device attributes (usually embeded within an inode)         *
 ***********************************************************************/
#define IOFUNC_NS_TIMESTAMP_SUPPORT
...
#define IOFUNC_ATTR_NS_TIMESTAMPS 0x00001000  /* nanosecond timestamp fields present in attribute structure */
...
#define IOFUNC_ATTR_DIRTY_SPARE   0x00000800  /* spare dirty flag */

#if __TIME_MAX < __TIME64_MAX
	#define FITS_IN_TIME_T(v)	((_Uint64t)(v) <= __TIME_MAX)
#else
	#define FITS_IN_TIME_T(v)	1
#endif
...
#else
	#define _IO_READ_GET_NBYTES(msg) ((msg)->i.nbytes | (((msg)->i.type == _IO_READ64) ? ((_Size64t)(msg)->i64.nbytes_hi << 32) : 0))
	#define _IO_WRITE_GET_NBYTES(msg) ((msg)->i.nbytes | (((msg)->i.type == _IO_WRITE64) ? ((_Size64t)(msg)->i64.nbytes_hi << 32) : 0))
#endif
...
#define IOFUNC_OCB_MMAP_UNIQUE      0x0004      /* hint for mmap handler to provide unique mapping */
...
typedef struct _iofunc_notify_event {
	...
	int							scoid;
	int							cnt;
	struct __sigevent32			old_event; /* maintain for backwards compatability */
	unsigned					flags;
	int							coid;
	union {
...
struct _iofunc_acl_funcs {
	...
	int							(*stat)(resmgr_context_t *ctp, IOFUNC_ATTR_T const *attr, struct stat *pstat);
	int							(*set)(IOFUNC_ATTR_T *attr, void const *data, int update_stats);
	int							(*size)(void const *data, size_t * size);
	int							(*endianise)(void *data, int from_non_native);
};
...
extern int iofunc_mount_init(IOFUNC_MOUNT_T *__mountp, const size_t __size);
extern int iofunc_mount_fini(IOFUNC_MOUNT_T *const __mountp);
extern int iofunc_mount_set_time(IOFUNC_MOUNT_T * const __mountp, const uint32_t __timeres);
...
extern void iofunc_attr_init_sized(iofunc_attr_t *__attr, mode_t __mode, iofunc_attr_t *__dattr, struct _client_info *__info, size_t __size);
...
extern int iofunc_stat_format(resmgr_context_t *__ctp, iofunc_attr_t *__attr, unsigned *__formatp, struct stat *__statp, unsigned *__lenp);
extern int iofunc_stat(resmgr_context_t *__ctp, iofunc_attr_t *__attr, struct stat *__statp);
...
#define IOFUNC_ABILITY_EXEC		"iofunc/exec"
...
typedef enum _iofunc_ability_e {
	...
	IOFUNC_ABILITYID_EXEC		/* ALLOW access for exec */
} iofunc_ability_e;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/c/public/sys/iofunc.h $ $Rev: 819647 $")
#endif
