...
/* ARMv7 has measurable function call overhead if we call __tls() */
 ...
 */
#define ISIRUPT_P(wtp)		(!ISSTACK_P(wtp) || (wtp)->intr_sighot == _ISIG_COLD)
#define ISIRUPT			isirupt()

#define ISSTART			(LIBC_TLS()->NW_TLS_TID < NW_DEF_TID_BASE_OFF)

static __inline int isirupt(void);

static __inline int
isirupt(void)
{
	struct nw_work_thread *wtp = WTP;
	return ISIRUPT_P(wtp);
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/io-pkt/sys/siglock.h $ $Rev: 800987 $")
#endif
