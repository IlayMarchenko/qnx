...
typedef enum {
	...
	_TRACEPARSER_INFO_SYSPAGE              = -1, /* ret. type (const syspage_entry*) casted to (const void*) */
	_TRACEPARSER_INFO_ENDIAN_CONV          = -2, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_NOW_CALLBACK_CLASS   = -3, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_NOW_CALLBACK_EVENT   = -4, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_PREV_CALLBACK_CLASS  = -5, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_PREV_CALLBACK_EVENT  = -6, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_PREV_CALLBACK_RETURN = -7, /* ret. type (const int*)           casted to (const void*)      */
	_TRACEPARSER_INFO_DEBUG                = -8, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_ERROR                = -9, /* ret. type (const unsigned*)      casted to (const void*)      */
	_TRACEPARSER_INFO_ERROR_STR            = -10,/* ret. type (const char*)          casted to (const void*)      */
	_TRACEPARSER_INFO_STATS                = -11,/* ret. type (const traceparser_kev_stats_st*) casted to (const void*)      */
	_TRACEPARSER_INFO_EVENT_ID             = -12,/* ret. type (const uint64_t*)      casted to (const void*)      */
	_TRACEPARSER_INFO_CLK_MSB              = -13,/* ret. type (const uint64_t*)      casted to (const void*)      */
	_TRACEPARSER_INFO_CLK_INTIAL           = -14,/* ret. type (const uint64_t*)      casted to (const void*)      */
	_TRACEPARSER_INFO_CLK                  = -15,/* ret. type (const uint64_t*)      casted to (const void*)      */
	_TRACEPARSER_INFO_CLK_DELTA            = -16,/* ret. type (const uint64_t*)      casted to (const void*)      */
	_TRACEPARSER_INFO_TIME_DELTA           = -17,/* ret. type (const char*)          casted to (const void*)      */
} info_modes_t;
...
extern const void* traceparser_get_info(struct traceparser_state* __state_ptr,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/traceparser/public/sys/traceparser.h $ $Rev: 812340 $")
#endif
