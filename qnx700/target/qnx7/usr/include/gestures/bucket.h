...
/**
 * @brief The number of gestures by which the gestures bucket grows each time it
 *        reaches its @c size.
 */
#define GESTURES_BUCKET_GROW_INCREMENT 4
/**
 * @brief The number of gestures by which the events bucket grows each time it
 *        reaches its @c size.
 */
 ...
 *
 * This structure represents the set of gestures awaiting
 * processing. The gesture bucket grows indefinitely by 
 * @c #GESTURES_BUCKET_GROW_INCREMENT every time it reaches @c size.
 */
typedef struct {
 ...
 *
 * This function removes all entries from the gestures array and sets the gesture
 * count and bucket size to zero.
 *
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/gestures/public/gestures/bucket.h $ $Rev: 812267 $")
#endif
