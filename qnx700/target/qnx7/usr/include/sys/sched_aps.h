 ...
 * The ap scheduler is optional and is present only if [module=aps] appears in your buildfile.
 *
...
enum {
	...
	SCHED_APS_SPARE1,	/* unused */
	SCHED_APS_SPARE2,	/* unused */
	SCHED_APS_SPARE3,	/* unused */
...
typedef struct {
	...
	_Uint16t		windowsize_ms;		/* length of averaging window used for scheduling */
	_Uint16t		reserved4;
	_Uint32t		reserved5;
	_Uint64t		reserved6;
} sched_aps_info;
...
typedef struct {
	_Int16t		windowsize_ms;	/*time over which scheduler will average cpu cycles and balance partitions to their
				* budgets as specified by  SCHED_APS_CREATE_PARTITION. By default 100ms. Set to -1 to skip. */
	_Int16t		reserved1;
 ...
 *
 * The name parameter is optional. If not provided, the name will be the same as the partition id.
 * The name may not start with a digit or contain a slash "/".
 *
...
typedef struct {
	...
	_Int16t		critical_priority;	/* threads at this or higher are critical, optional, set to -1 or 0 to skip*/
	_Uint16t 	budget_percent_scale; /* number of digits to the right of the decimal point */
	_Uint64t	reserved1;
	_Uint32t	reserved2;
	/* output parms */
...
/* SCHED_APS_QUERY_PARTITION
 *
 * Fills in the following structure to describe one partition. To convert the critical_budget_cycles field to milliseconds,
 ...
 *
 * If the new budget value is different from current, the difference is either taken from, or returned to, the
...
typedef struct {
		...
		_Int16t		new_critical_priority;	/* optional, set to -1 or 0 to skip */
		_Uint16t 	budget_percent_scale; /* number of digits to the right of the decimal point */
		_Int32t 	reserved1;
		_Int64t 	reserved2;
 ...
 *
 *  Note that dynamic_windowsize_cycles may differ from nominal window size. Use dynamic_windowsize_cycles to
 *  convert run_time_cycles to percentage.
 *
...
typedef struct {
	/* out parms */
	_Uint64t		run_time_cycles;		/* cpu execution time during last scheduling window */
	_Uint64t		critical_time_cycles;	/* time billed as critical during last scheduling window */
	_Uint64t		reserved1;
	_Uint64t		reserved2;
	_Uint64t		reserved3;
	_Uint64t		reserved4;
	...
	_Uint32t		reserved5;
	_Uint64t		dynamic_windowsize_cycles;	/* length of last averaging window used for scheduling */
	_Uint64t		reserved6;
	/* in parm */
...
/*	SCHED_APS_OVERALL_STATS
 *
 * Returns instantaneous values of other dynamic scheduler states.
 *
 * returns:		EOK, ENOSYS, EINVAL as described for SchedCtl
 */
...
#define APS_QCRIT_RESERVED			0x00000001u /* unused */
#define APS_QCRIT_RUNNING_CRITICAL	0x00000002u
#define APS_QCRIT_BILL_AS_CRITICAL	0x00000004u
 ...
 * all met:
 * 	1. the partition of the running thread has a critical budget greater than zero
 * 	2. the the running thread has critical priority
 * 	3. the partition must be out of percentage-cpu budget
 ...
 * 2. If the user has set a sigevent via procmgr_event_notify(PROCMGR_EVENT_APS_BANKRUPTCY), deliver the event.
  *
 * In addition the following responses are configurable.
 */
...
#define SCHED_APS_BNKR_RESERVED		0x000000002 /* unused */
 ...
 * of the partition being modified. 'Modify' means change a partition's budget or critical budget.
 */
 ...
 * you have set up a set of partitions. Once locked, SCHED_APS_JOIN_PARTITION may still be used.
 */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/services/system/public/sys/sched_aps.h $ $Rev: 816760 $")
#endif
