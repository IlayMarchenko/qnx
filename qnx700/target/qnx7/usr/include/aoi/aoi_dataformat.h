...
#define AOIFOURCC_METADATA_STORAGE    "TmpF" /* Temporary path for storing metadata payload into file system (ex: a picture). The filemane is in ASCII format. */
...
{
	uint16_t n;      /* aspect ratio numerator (width)   */
	uint16_t d;      /* aspect ratio denominator (height) */
} AOAspectRatio_t;

/* Video colour spaces for use with the colour_space member below. */
#define MEDIA_COLOUR_SPACE_UNCORRECTED   0
#define MEDIA_COLOUR_SPACE_BT601_FULL    1
...
typedef struct
{
	...
	int16_t rotation; /* rotation angle when rendering video frames. can be 0/90/180/270 */
	uint8_t color_space;  /* Indicate the video color space */
	int8_t  reserve2;
} AOVideoFormat_t;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/aoi/public/aoi/aoi_dataformat.h $ $REV$")
#endif
